  ...
  const sincronizar = useCallback(async (id: string, numero: string, silencioso = false) => {
    ...
    try {
      ...
      await updateProcesso.mutateAsync({
        ...
        vara:        norm.orgaoJulgador,
        partes:      [norm.autor, norm.reu].filter(x => x !== "—").join(" × ") || null,
        advogados:   null,
      ...
      if (norm._movimentacoes.length > 0 && user) {
        await supabase.from("movimentacoes").delete().eq("processo_id", id);
        await supabase.from("movimentacoes").insert(
          norm._movimentacoes.slice(0, 100).map((m: any) => ({
            processo_id: id,
            user_id: user.id,
            data: m.dataISO || new Date().toISOString().slice(0, 10),
            titulo: m.tipo,
            descricao: m.complementosTabelados?.map((c: any) => c.nome).join("; ") || m.complementoLivre || null,
            analise_ia: null,
          }))
        );
      }
      ...
      setProcessos(prev => prev.map(p =>
        p.id === id ? { ...p, ...norm, classe: norm.classe, assunto: norm.assunto, tribunal: norm.tribunalNome, vara: norm.orgaoJulgador } : p
      ));
      if (panelProcesso?.id === id) setPanelProcesso(p => p ? { ...p, ...norm } : p);

      if (!silencioso) toast({ title: `✅ ${norm._movimentacoes.length} movimentação(ões) sincronizada(s)!`, description: norm.tribunalNome });
