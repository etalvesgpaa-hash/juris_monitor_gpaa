...
import {
  ...
  Shield, BarChart2, AlertTriangle
} from "lucide-react";
...
export function AdminPage() {
  ...
  const nomeResponsavel = (userId: string) =>
    profileMap[userId]?.full_name || profileMap[userId]?.escritorio || userId.slice(0, 8) + "…";
  ...
  const handleExportCSV = () => {
    const rows = [
      ["Nº CNJ", "Classe", "Tribunal", "Comarca", "Status", "Responsável", "Intimações", "Tarefas", "Criado em"],
      ...processos.map(p => [
        p.numero_cnj,
        p.classe ?? "",
        p.tribunal ?? "",
        ...
        intimacoes.filter(i => i.processo_id === p.id).length,
        tarefas.filter(t => t.processo_id === p.id).length,
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-border print:hidden">
        {(["overview", "processos", "intimacoes", "relatorio"] as const).map(t => (
          ...
          >
            {{ overview: "Por Usuário", processos: "Processos", intimacoes: "Intimações", relatorio: "Relatório Completo" }[t]}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && !loading && (
        <div className="space-y-3">
          {profiles.length === 0 && (
            <p className="text-center text-muted-foreground py-10 text-sm">Nenhum usuário cadastrado.</p>
          )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm text-foreground flex items-center gap-2">
                      ...
                      {profile.is_admin && (
                        <span className="bg-violet-100 text-violet-700 text-[0.6rem] px-1.5 py-0.5 rounded font-bold">ADMIN</span>
                      )}
                    </div>
                          ...
                          {userIntimacoes.slice(0, 5).map(i => (
                            <div key={i.id} className="rounded-lg border border-border bg-background px-3 py-2 text-xs flex items-start gap-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-muted-foreground">{fmtDate(i.data_publicacao)} · Prazo: {fmtDate(i.prazo)}</div>
                              </div>
                              {statusBadge(i.status)}
                            </div>
                          ))}
                          {userIntimacoes.length > 5 && (
                            <p className="text-xs text-muted-foreground pl-1">+ {userIntimacoes.length - 5} mais…</p>
                          )}
                        </div>
                      </div>
                    )}
      ...
      {tab === "processos" && !loading && (
        <div className="rounded-xl border border-border overflow-hidden">
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/50 sticky top-0">
                <tr>
                  ...
                  <th className="text-left px-3 py-2.5 font-semibold">Intimações</th>
          ...
          <div className="px-3 py-2 border-t border-border bg-muted/20 text-xs text-muted-foreground">
            {filteredProcessos.length} processo(s) exibido(s)
          </div>
        </div>
      )}

      {/* ══════════════════════════════════════════════════════════
          ABA: INTIMAÇÕES (tabela completa)
      ══════════════════════════════════════════════════════════ */}
      {tab === "intimacoes" && !loading && (
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          <div className="px-3 py-2 border-t border-border bg-muted/20 text-xs text-muted-foreground">
            {filteredIntimacoes.length} intimação(ões) exibida(s)
          </div>
        </div>
      )}

      {/* ══════════════════════════════════════════════════════════
          ABA: RELATÓRIO COMPLETO (formato imprimível)
      ══════════════════════════════════════════════════════════ */}
      {tab === "relatorio" && !loading && (
        <div ref={printRef} className="space-y-8 print:space-y-6">
          {/* Cabeçalho do relatório */}
          <div className="rounded-xl border border-border bg-card p-6 print:border-none print:p-0">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="font-display font-extrabold text-2xl text-foreground">Relatório Geral — JurisMonitor</h2>
                <p className="text-sm text-muted-foreground mt-1">
                  Gerado em {new Date().toLocaleString("pt-BR")} por {user?.user_metadata?.full_name || user?.email}
                </p>
              </div>
          ...
          {profiles.map(profile => {
            ...
            return (
              <div key={uid} className="rounded-xl border border-border bg-card overflow-hidden print:break-inside-avoid">
                {/* Header usuário */}
                <div className="bg-primary px-5 py-3 flex items-center gap-3">
                  ...
                  <div className="hidden sm:flex gap-4 text-xs text-primary-foreground/70 shrink-0">
                    ...
                    <span><b className="text-primary-foreground">{userTarefas.length}</b> tarefas</span>
                  </div>
                </div>
                ...
                <div className="p-5 space-y-5">
                        ...
                        <table className="w-full text-xs">
                          <thead className="bg-muted/40">
                            <tr>
                              ...
                              <th className="text-left px-3 py-2 font-semibold">Classe</th>
                              <th className="text-left px-3 py-2 font-semibold">Tribunal</th>
                              <th className="text-left px-3 py-2 font-semibold">Vara</th>
                              <th className="text-left px-3 py-2 font-semibold">Status</th>
                          ...
                          <tbody>
                            {userProcessos.map(p => {
                              const procIntimacoes = intimacoes.filter(i => i.processo_id === p.id);
                              return (
                                <>
                                  ...
                                  {procIntimacoes.map(i => (
                                    <tr key={i.id} className="bg-amber-50/50 border-t border-amber-100">
                                      <td colSpan={6} className="px-4 py-1.5">
                                        <div className="flex items-start gap-2 text-[0.7rem]">
                                          ...
                                          <div>
                                            ...
                                            <span className="text-muted-foreground">Publicação: {fmtDate(i.data_publicacao)}</span>
                                            <span className="mx-1 text-muted-foreground">·</span>
                                            <span className="text-muted-foreground">Prazo: {fmtDate(i.prazo)}</span>
                                            <span className="mx-1">{statusBadge(i.status)}</span>
                                            {i.conteudo && (
                                              <div className="text-muted-foreground mt-0.5 line-clamp-2">{i.conteudo}</div>
                                            )}
                                          </div>
                                        </div>
                                      </td>
                  ...
                  {/* Intimações sem processo vinculado */}
                  {(() => {
                    ...
                    return (
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                          <Bell className="h-3.5 w-3.5" /> Intimações sem processo vinculado
                        </h4>
                        <div className="space-y-1.5">
                          {soltas.map(i => (
                            <div key={i.id} className="rounded-lg border border-amber-200 bg-amber-50/50 px-3 py-2 text-xs flex items-start gap-2">
                              <AlertTriangle className="h-3.5 w-3.5 text-amber-500 mt-0.5 shrink-0" />
                              <div className="flex-1">
                                <div className="font-semibold">{i.numero_processo || "Sem número"} — {i.tipo || "—"}</div>
                                <div className="text-muted-foreground">
                                  Publicação: {fmtDate(i.data_publicacao)} · Prazo: {fmtDate(i.prazo)} · {i.origem.toUpperCase()}
                                </div>
                  ...
                  {userProcessos.length === 0 && userIntimacoes.length === 0 && (
                    <p className="text-xs text-muted-foreground italic">Usuário sem processos ou intimações cadastrados.</p>
                  )}
                </div>
              </div>
            );
      ...
      <style>{`
        @media print {
          ...
          .print\\:break-inside-avoid * { visibility: visible !important; }
          [data-print-area],
          [data-print-area] * { visibility: visible !important; }
